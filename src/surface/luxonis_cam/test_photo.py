...
class RerunNode(dai.node.ThreadedHostNode):
    ...
    def run(self) -> None:
        ...
        rr.log("world/ground", rr.Boxes3D(half_sizes=[1.0, 1.0, 0.00001]))
        while True:
            try:
...
with dai.Pipeline() as p:
    ...
    if platform == dai.Platform.RVC4:
        out = left.requestOutput((640,400), dai.ImgFrame.Type.RGB888i)
        align = p.create(dai.node.ImageAlign)
        stereo.depth.link(align.input)
        out.link(align.inputAlignTo)
        align.outputAligned.link(rgbd.inDepth)
    else:
        out = left.requestOutput((640,400), dai.ImgFrame.Type.RGB888i, dai.ImgResizeMode.CROP, 30, True)
